...
class LineCostFunctor{
    public:
    LineCostFunctor(const Eigen::Vector3d &point,const Eigen::Matrix<double,6,1> &coeff):
    point_(point),
    line_coeffs_(coeff){}
    template<class T>
    bool operator()(const T* angle,const T*  Trans,T* residual) const{
        ...
        Eigen::Matrix<T,3,1> direction(T(line_coeffs_(3)),T(line_coeffs_(4)),T(line_coeffs_(5)));
        Eigen::Matrix<T,3,1> start_point(T(line_coeffs_(0)),T(line_coeffs_(1)),T(line_coeffs_(2)));
        Eigen::Matrix<T,3,1> distance = (Eigen::Matrix<T,3,3>::Identity(3,3)-direction*direction.transpose())*(Porject_vec-start_point);
        residual[0] = distance.norm()*distance.norm() ;
        return true;
    }
    const Eigen::Vector3d point_;
    const Eigen::Matrix<double,6,1> line_coeffs_;
};
int main(int argc, char *argv[])
{
    ...
    Eigen::Matrix<double,6,1> coeff[2];
    ...
    pcl::PointCloud<pcl::PointXYZ> initial_out;
    for(int i=0;i<2;i++)
    {
        for(auto pt: cloud_out[i])
        {
            ...
            pcl::PointXYZ pttt(out_pt(0),out_pt(1),out_pt(2));
            initial_out.push_back(pttt);
        }
    }
    for(int i=0;i<2;i++)
    {
    ...
    pcl::io::savePCDFile("/home/ramlab/Documents/catkin_ws/src/merge_cloud/transformer.pcd",final_out);
    return 0;
}
